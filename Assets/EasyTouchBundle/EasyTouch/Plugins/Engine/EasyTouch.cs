...
namespace HedgehogTeam.EasyTouch
{
    ...
    public class EasyTouch : MonoBehaviour
    {
        ...
        /// <summary> Enables or disables Easy Touch </summary>
        public bool enable;             
        /// <summary> Enables or disables Unity remote </summary>
        public bool enableRemote;           

        // General gesture properties
        public GesturePriority gesturePriority;
        public float StationaryTolerance;
        /// <summary> The time required for the detection of a long tap. </summary>
        public float longTapTime ;          
        /// <summary> Determines the accuracy of detecting a drag movement 0 => no precision 1=> high precision. </summary>
        public float swipeTolerance;        
        /// <summary> The minimum length for a pinch detection. </summary>
        public float minPinchLength;            
        /// <summary> The minimum angle for a twist detection. </summary>
        ...
        private PickedData pickedData = new PickedData();
        ...
        void Update()
        {
            ...
            if (enable && EasyTouch.instance == this) {
                ...
                // two fingers gesture
                twoFinger.oldPickedData = twoFinger.pickedData;
            ...
            if (fingers[ fingerIndex ].gesture == GestureType.None) {
                ...
                if (autoSelect) {
                    if (GetPickedGameObject( fingers[ fingerIndex ] )) {
                        fingers[ fingerIndex ].pickedData = pickedData;
                    }
                }
            ...
            if (fingers[ fingerIndex ].phase != TouchPhase.Ended && fingers[ fingerIndex ].phase != TouchPhase.Canceled) {
                ...
                if (((fingers[ fingerIndex ].gesture == GestureType.Acquisition || fingers[ fingerIndex ].gesture == GestureType.LongTap) && fingers[ fingerIndex ].phase == TouchPhase.Moved && gesturePriority == GesturePriority.Slips)
                    ...
                    if (fingers[ fingerIndex ].gesture == GestureType.LongTap) {
                        ...
                        if (fingers[ fingerIndex ].pickedObject != null) {
        ...
        private void CreateGesture( int touchIndex, EvtType message, Finger finger, SwipeDirection swipe, float swipeLength, Vector2 swipeVector )
        {
            ...
            if (firingEvent) {
                ...
                //Debug.Log( "EasyTouch: CreateGesture() enableUIMode: " + enableUIMode + " firingEvent: " + firingEvent + " evtType: " + message);
            ...
            //Debug.Log( "EasyTouch: CreateGesture() Event Type: " + message );
            ...
            if (twoFinger.currentGesture == GestureType.None) {
                ...
                if (autoSelect) {
                    if (GetTwoFingerPickedObject()) {
                        twoFinger.pickedData = pickedData;
                    } else {
                ...
                if (allowUIDetection) {
                    if (GetTwoFingerPickedUIElement()) {
                        twoFinger.pickedUIElement = pickedData.pickedObj;
                ...
                if (move && twoFinger.currentGesture != GestureType.Tap) {
                    ...
                    if (enable2FingersSwipe) {
                        ...
                        if (dot > 0) {
                            ...
                            if (twoFinger.pickedData != null && !twoFinger.dragStart && !alwaysSendSwipe) {
                            ...
                            } else if (twoFinger.pickedData == null && !twoFinger.swipeStart) {
                                           ...
                                           float actionTime, SwipeDirection swipe, float swipeLength, Vector2 swipeVector, float twist, float pinch, float twoDistance )
        {
            ...
            gesture.pickedData = twoFinger.pickedData;
            ...
            if (allowUIDetection && autoUpdatePickedUI) {
                if (message != EvtType.On_Drag && message != EvtType.On_DragEnd && message != EvtType.On_Twist && message != EvtType.On_TwistEnd && message != EvtType.On_Pinch && message != EvtType.On_PinchEnd
                    ...
                    if (message == EvtType.On_SimpleTap2Fingers)
                        ...
                        if (GetTwoFingerPickedUIElement()) {
                            gesture.pickedUIElement = pickedData.pickedObj;
        ...
        private bool GetTwoFingerPickedObject()
        {
            ...
            if (twoFingerPickMethod == TwoFingerPickMethod.Finger) {
                if (GetPickedGameObject( fingers[ twoFinger.finger0 ], false )) {
                    GameObject tmp = pickedData.pickedObj;
                    if (GetPickedGameObject( fingers[ twoFinger.finger1 ], false )) {
                        if (tmp == pickedData.pickedObj) {
                            returnValue = true;
                        }
                    }
                }
        ...
        private bool GetTwoFingerPickedUIElement()
        {
            ...
            if (twoFingerPickMethod == TwoFingerPickMethod.Finger) {
                if (IsScreenPositionOverUI( fingers[ twoFinger.finger0 ].position )) {
                    ...
                    if (IsScreenPositionOverUI( fingers[ twoFinger.finger1 ].position )) {
                        ...
                        if (tmp2 == tmp || tmp2.transform.IsChildOf( tmp.transform ) || tmp.transform.IsChildOf( tmp2.transform )) {
                            //pickedData.pickedObj = tmp;
                            pickedData.isGUI = true;
                            returnValue = true;
                        }
                    }
                }
                ...
                if (IsScreenPositionOverUI( twoFinger.position )) {
                    //pickedData.pickedObj = GetFirstUIElementFromCache();
                    pickedData.isGUI = true;
                    returnValue = true;
                }
            }
        ...
        private void RaiseEvent( EvtType evnt, Gesture gesture )
        {
            ...
            if (result > -1) {
                ...
                _currentGestures[ result ].pickedData = gesture.pickedData;
            ...
            if (touchCameras.Count > 0) {
                ...
                for (int i = 0; i < touchCameras.Count; i++) {
                    if (touchCameras[ i ].camera != null && touchCameras[ i ].camera.enabled) {
                        ...
                        if (GetGameObjectAt( pos, touchCameras[ i ] )) {
                            return true;
                        }
        ...
        private bool GetGameObjectAt( Vector2 position, ECamera ecam )
        {
            var cam = ecam.camera;
            var isGuiCam = ecam.guiCamera;

            Ray ray = cam.ScreenPointToRay( position );

            // TODO - Cache the RaycastHit and PickedObject for the entire current frame for this Finger 
            //        (Make the assumption that we can only possibly hit one object per frame)?
            if (ecam.useGlobalLayers) 
            {
                //--------------------------
                // Use Global Layer Masks
                //--------------------------
                if (enable2D) {
                    RaycastHit2D[] hit2D = new RaycastHit2D[1];
                    if (Physics2D.GetRayIntersectionNonAlloc( ray, hit2D, float.PositiveInfinity, pickableLayers2D ) > 0) {
                        pickedData.camera = cam;
                        pickedData.isGUI = isGuiCam;
                        pickedData.isHit2D = true;
                        pickedData.hit2D = hit2D[ 0 ];
                        return true;
                    }
                }

                // Always Raycast Global 3D
                RaycastHit hit;
                if (Physics.Raycast( ray, out hit, float.MaxValue, pickableLayers3D )) {
                    pickedData.camera = cam;
                    pickedData.isGUI = isGuiCam;
                    pickedData.isHit2D = false;
                    pickedData.hit = hit;
                    return true;
                }
            ...
            {
                //--------------------------
                // Use Camera Layer Masks
                //--------------------------
                if (ecam.enable2D) {
                    RaycastHit2D[] hit2D = new RaycastHit2D[1];
                    if (Physics2D.GetRayIntersectionNonAlloc( ray, hit2D, float.PositiveInfinity, ecam.pickableLayers2D ) > 0) {
                        pickedData.camera = cam;
                        pickedData.isGUI = isGuiCam;
                        pickedData.isHit2D = true;
                        pickedData.hit2D = hit2D[0];
                ...
                if (ecam.enable3D) {
                    RaycastHit hit;
                    if (Physics.Raycast( ray, out hit, float.MaxValue, ecam.pickableLayers3D )) {
                        pickedData.camera = cam;
                        pickedData.isGUI = isGuiCam;
                        pickedData.isHit2D = false;
                        pickedData.hit = hit;
        ...
        public static GameObject GetCurrentPickedObject( int fingerIndex, bool isTwoFinger )
        {
            ...
            if (EasyTouch.instance != null) {
                ...
                if ((finger != null || isTwoFinger) && EasyTouch.instance.GetPickedGameObject( finger, isTwoFinger )) {
                    return EasyTouch.instance.pickedData.pickedObj;
                } else {
                    return null;
                }
            } else {
        ...
        {
            var etInstance = EasyTouch.instance;

            if (etInstance != null) 
            {
                if (isTwoFinger)
                    position = etInstance.twoFinger.position;
                if (etInstance.touchCameras.Count > 0) 
                {
                    for (int i = 0; i < etInstance.touchCameras.Count; i++) 
                    {
                        if (etInstance.touchCameras[ i ].camera != null && etInstance.touchCameras[ i ].camera.enabled) {
                            if (etInstance.GetGameObjectAt( position, etInstance.touchCameras[ i ] )) {
                                return etInstance.pickedData.pickedObj;
                            } 
                        }
                    }
                }
