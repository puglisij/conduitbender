...
namespace UnityEngine.UI.Extensions
{
    public static class uGUITools
    {
        ...
        [MenuItem( "uGUI/Replace Text with Typogenic %\"" )]
        static void ReplaceUnityFonts()
        {
            // Returns the active transform. (The one shown in the inspector)
            var activeTrans = Selection.activeTransform;
            if (activeTrans == null) { return; }

            // Save the Scene
            var saved = EditorSceneManager.SaveScene( EditorSceneManager.GetActiveScene() );
            if(!saved) {
                Debug.LogError( "uGUI/Replace Text with Typogenic. Failed to save current scene before proceeding." );
                return;
            }

            var answer = EditorUtility.DisplayDialogComplex( "Replace Text with Typogenic Font",
                "Do you want to perform this on all children of the current selected object?",
                "Yes", "No", "Cancel" );
            if(answer == 2) {
                return;
            }
